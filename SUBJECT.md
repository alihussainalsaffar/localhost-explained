...
> Copied for offline reference from the official 01-edu subject:
> <https://github.com/01-edu/public/tree/master/subjects/localhost>
> If the two ever disagree, the upstream page wins.
...
## How this repository answers each requirement

| Requirement | Where it lives |
| ----------- | -------------- |
| Written in Rust, only `libc` | [Cargo.toml](Cargo.toml) — one dependency |
| Never crashes | `catch_unwind` around every handler, [src/server.rs](src/server.rs) |
| Requests time out | 15 s sweep + `408`, [src/server.rs](src/server.rs) |
| Multiple ports / servers | grouping in `server::run`, [config/ports.conf](config/ports.conf) |
| One process, one thread | no `thread::spawn`, no `fork` outside CGI |
| HTTP/1.1 | [src/http.rs](src/http.rs) |
| GET, POST, DELETE | `Handler::dispatch`, [src/handlers.rs](src/handlers.rs) |
| File uploads | `save_multipart`, [src/handlers.rs](src/handlers.rs) |
| Cookies and sessions | `ensure_session` / `touch_session`, [src/handlers.rs](src/handlers.rs) |
| Error pages 400/403/404/405/413/500 | [www/errors/](www/errors) + `error_page` directives |
| One epoll for everything | the single `Epoll` in [src/server.rs](src/server.rs) |
| Non-blocking I/O | `set_nonblocking` on every socket |
| Chunked and unchunked bodies | `decode_chunked`, [src/http.rs](src/http.rs) |
| CGI | [src/cgi.rs](src/cgi.rs), scripts in [www/cgi/](www/cgi) |
| Configuration file | [src/config.rs](src/config.rs), [config/](config) |
| Tests | [tests/run_tests.py](tests/run_tests.py), [tests/stress.py](tests/stress.py) |
| Bonus: a second CGI script | [www/cgi/time.py](www/cgi/time.py) |
